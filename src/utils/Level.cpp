	...
	namespace utils {
		...
		Level::Level(std::string config_filename) : m_background_texture(std::make_unique<sf::Texture>()) {
			...
			try {
				...
				error::ConfigFileException err(config_filename, "background");
				std::cout << err.what() << std::endl;
			}

			try {
				if (!m_background_texture->loadFromFile(background_texture_filename))
					throw error::MediaFileException(background_texture_filename);
			} catch (std::exception& e) {
			...
			try {
				...
				error::ConfigFileException err(config_filename, "border_config_file");
				std::cout << err.what() << std::endl;
			}
				...
				try {
					for (auto enemy : wave["enemies"]) {
						...
						try {
							...
							throw error::ConfigFileException(config_filename, "amount");
						}

						try {
							enemy_config = enemy["enemy_config_file"];
						} catch (std::exception& e) {
							throw error::ConfigFileException(config_filename, "enemy_config_file");
						}
				...
				try {
					for (auto obstacle : wave["obstacles"]) {
						...
						try {
							...
							throw error::ConfigFileException (config_filename, "obstacle_config_file");
						}
